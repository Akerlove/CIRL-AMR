...
from models.networks import resnet18_1d, Masker, Classifier, ProjectionHead
...
def info_nce_loss(z_i, z_j, temperature=0.07):
    ...
    z_i, z_j: (B, D) - Projections of original and augmented views
    """
    ...
    # We want to maximize similarity between (i, i+B) and (i+B, i)
    
    # SimCLR style implementation
    # Compute logits
...
def save_snr_results(log_dir, snr_accs):
    ...
    plt.plot(snrs, accs, 'o-', linewidth=2, label='CIRL+CL')
...
def train(args):
    ...
    exp_dir = os.path.join(args.log_dir, f"exp_{timestamp}")
    os.makedirs(exp_dir, exist_ok=True)
    print(f"Experiment logs will be saved to: {exp_dir}")
    ...
    proj_head = ProjectionHead(feature_dim).to(device)
    
    # Optimizers
    # Step 1: Encoder + Classifiers + Projection Head
    optimizer_enc = optim.Adam(
        list(backbone.parameters()) + list(classifier.parameters()) + list(classifier_ad.parameters()) + list(proj_head.parameters()),
        lr=args.lr,
        weight_decay=1e-4
    )
    ...
    for epoch in range(args.epochs):
        ...
        for x_orig, x_aug, labels, snrs in pbar:
            ...
            z_orig = proj_head(f_orig)
            z_aug = proj_head(f_aug)
            ...
            # --- Step 1: Update Encoder & Classifiers & Projection Head ---
            # Minimize L_sup + L_inf + L_fac + L_cl
            loss_sup = criterion(pred_sup, labels)
            loss_inf = criterion(pred_inf, labels)
            loss_fac = factorization_loss(f_orig, f_aug)
            loss_cl = info_nce_loss(z_orig, z_aug)
            
            loss_step1 = loss_sup + args.alpha * loss_inf + args.beta * loss_fac + args.delta * loss_cl
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(backbone.state_dict(), os.path.join(exp_dir, 'best_backbone.pth'))
            torch.save(masker.state_dict(), os.path.join(exp_dir, 'best_masker.pth'))
            torch.save(classifier.state_dict(), os.path.join(exp_dir, 'best_classifier.pth'))
            torch.save(proj_head.state_dict(), os.path.join(exp_dir, 'best_proj_head.pth'))
            
            # Save SNR results for the best model
            save_snr_results(exp_dir, snr_accs)
            # Save Confusion Matrix for the best model (0dB and 18dB)
            save_confusion_matrix(exp_dir, preds, labels_gt, snrs_gt, train_dataset.classes, target_snrs=[0, 18])
